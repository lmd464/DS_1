...
public class AppController {
	...
	private void removeCoin() {
		...
		// 음수를 입력받았을 때 : 맨 앞의 코인 제거
		if(coinValue < 0) {
			Coin removedCoin = this._coinBag.removeAny();
			if(removedCoin == null) {
				this._appView.outputLine("가방에 동전이 존재하지 않습니다.");
			}
			else {
				this._appView.outputLine("가방에서 동전 " + removedCoin.value() + " 를 제거하였습니다." );
			}
		}
		
		// 음수가 아닌 값을 입력받았을 때 : 해당 값의 코인 제거
		else {
